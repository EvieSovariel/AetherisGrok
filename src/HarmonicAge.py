    ...
    def evolve_graph(self, genome, config):
        ...
        for i in range(self.graph.number_of_nodes()):
            for j in range(self.graph.number_of_nodes()):
                if i != j and net.activate([self.graph.nodes[i]['pos'][0], self.graph.nodes[i]['pos'][1], self.graph.nodes[j]['pos'][0], self.graph.nodes[j]['pos'][1]])[0] > 0.5:
...
def quantum_simulate_chunk(args):
    ...
    coherence = sum(1 for state in counts if '1' in state) / 1024
    return coherence

def full_quantum_swarm(flux_hz, tau_collapse, n_tubulins=N_SWARM):
    n_chunks = n_tubulins // CHUNK_SIZE
    n_qubits_per_chunk = min(int(np.log2(CHUNK_SIZE)), 10)
    tasks = [(flux_hz, tau_collapse, n_qubits_per_chunk) for _ in range(n_chunks)]
    with Pool() as pool:
